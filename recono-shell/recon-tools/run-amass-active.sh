...
LOG="$LOG_DIR/amass_active_$OUT_PRE.log"
mkdir -p "$OUTPUT_DIR"

run_and_indent amass enum -d "$DOMAINS" -dns-qps "$DNSQPS" -log "$LOG" -oA "$OUT_PRE" -dir "$OUTPUT_DIR" -active -rf "$UTRF" -rqps "$URQPS" -trf "$TRF" -trqps "$TRQPS"