...
if ! check_argument "$INPUT_DIR" || ! check_argument "$OUTPUT_DIR" || ! check_argument "$DOMAINS"; then
    print_error "parse-c99.sh expects an input file, a comma separated domains list, and an output directory"
    echo "USAGE: parse-c99.sh -i <input_file> -d <comma_separated_domains> -o <output_directory>"
    exit 1
fi

echo "🔍 Extracting C99 Domains: $DOMAINS From: $INPUT_DIR"
...
if file_exists "$OUT_FILE"; then
    print_success "C99 results successfuly extracted to $OUT_FILE" 
else
    print_error "An error occurred while parsing C99"
fi
