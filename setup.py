...
def check_base_requirements():
    if sys.version_info < (3, 10):
        common.handle_error('ERROR: This script ust be run using python 3.10 or higher.')
        sys.exit(1)
...
def initialize_file_structure_requirements():
    ...
    try:
        print(f'Copying {script_root} to {config_dir}')
        shutil.copytree(script_root, config_dir, ignore=shutil.ignore_patterns('.git', '.idea', 'knockpy.egg-info','__pycache__', 'SubScraper.egg-info', 'example-config.toml', 'dist', 'build'))
    except Exception as e:
        handle_error(f'Problem ocurred while copying {script_root} to {config_dir}', e)
...
def install_shell_tools():
    ...
    test_wordlist = wordlist_path = os.path.join(config_dir, 'wordlists', 'test_install.txt')
